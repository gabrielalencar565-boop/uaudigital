...
-- Atualiza recompute_metas_prazos para ignorar etapas pdf, alteracoes, agendamento
...
BEGIN
  ...
  WHERE t.status = 'concluido'
    ...
    AND t.stage NOT IN ('pdf', 'alteracoes', 'agendamento')
...
-- Atualiza recompute_all_scores para ignorar etapas pdf, alteracoes, agendamento
...
BEGIN
  ...
  -- Conta tarefas EXCLUINDO pdf, alteracoes, agendamento
  SELECT 
    COUNT(*),
  ...
  -- METAS/PRAZOS (exclui pdf, alteracoes, agendamento)
  SELECT COALESCE(
    SUM(
      COALESCE(o.override_points,
        CASE
  ...
  -- APRENDIZADO CONTÍNUO (exclui pdf, alteracoes, agendamento)
  SELECT CASE

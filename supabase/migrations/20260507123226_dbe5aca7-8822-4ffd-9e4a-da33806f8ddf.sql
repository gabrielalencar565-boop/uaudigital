...
-- Remove the freelancer early-return so Freelancer client tasks also generate
-- scoring snapshots and appear in reports.
-- We fetch the full current function and recreate it without the freelancer guard.

CREATE OR REPLACE FUNCTION public.pm_sync_stage_completion(_pm_task_id uuid, _completed_stage text, _user_id uuid DEFAULT NULL::uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- REMOVED: freelancer sentinel guard (freelancer tasks now score normally)
  ...
  IF v_child_count > 0 AND NOT v_force_parent_assignee THEN
    ...
    -- watcher scoring for multi-assignee
  ...
  ELSE
    -- No children or force-parent-assignee stages
    ...
    FOREACH v_user IN ARRAY v_all_users LOOP
      v_desc_key := 'pm:' || _pm_task_id::text || ':' || _completed_stage || ':' || v_user::text;
  ...
  -- Recompute metas_prazos
  FOREACH v_user IN ARRAY v_all_users LOOP
    PERFORM public.recompute_metas_prazos(v_user, v_year, v_month);
  END LOOP;

  IF v_child_count > 0 AND NOT v_force_parent_assignee THEN
    FOREACH v_current_assignee IN ARRAY v_all_assignees LOOP
      IF NOT (v_current_assignee = ANY(v_all_users)) THEN
        PERFORM public.recompute_metas_prazos(v_current_assignee, v_year, v_month);
      END IF;
    END LOOP;
    IF v_watchers IS NOT NULL AND array_length(v_watchers, 1) > 0 THEN
      FOREACH v_watcher IN ARRAY v_watchers LOOP
        IF NOT (v_watcher = ANY(v_all_users)) AND NOT (v_watcher = ANY(v_all_assignees)) THEN

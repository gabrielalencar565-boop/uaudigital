...
BEGIN
  ...
  -- Before April 2026: flat 1 point per task (old criterion)
  v_use_old := (_year < 2026 OR (_year = 2026 AND _month < 4));

  IF v_use_old THEN
    -- Old scoring: 1 point per on-time completed task, -1 for late
    SELECT COALESCE(
      SUM(
        COALESCE(
  ...
  ELSE
    -- New scoring: weighted by stage config + tag points from April 2026
    SELECT COALESCE(
      SUM(
        COALESCE(
          o.override_points::numeric,

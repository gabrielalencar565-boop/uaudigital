...
CREATE OR REPLACE FUNCTION public.pm_sync_stage_completion(_pm_task_id uuid, _completed_stage text, _user_id uuid, _scoring_user_ids uuid[] DEFAULT NULL)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Build list of users to score
  -- Use explicit list if provided (avoids race condition), otherwise fallback to _user_id
  ...
  -- Also collect unique assignees from child tasks (subtasks)
  -- Each child's assignee should get scored individually
  FOR v_child IN
  ...
  -- ═══ MAGIC NUMBER SYNC (skip if extra demand) — only once ═══
  ...
  -- ═══ PERFORMANCE SYNC — score ALL users ═══
  FOREACH v_uid IN ARRAY v_all_users LOOP
    ...
    IF v_existing_task_id IS NULL THEN
      -- Check old format (without user suffix)

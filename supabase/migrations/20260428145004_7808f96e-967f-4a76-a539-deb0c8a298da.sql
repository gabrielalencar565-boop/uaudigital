...
BEGIN
  ...
  IF v_child_count > 0 AND NOT v_force_parent_assignee THEN
    ...
    -- NEW: Score watchers of the parent task too. Each watcher gets the full
    -- subtask quantity (to match the same point share as the main assignee
    -- when the parent has a single assignee), but watchers already counted
    -- in the per-subtask distribution are skipped to avoid double counting.

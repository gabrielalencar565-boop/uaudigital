...
BEGIN
  ...
  LOOP
    ...
    -- Penalidade aplicada UMA VEZ na tarefa pai (não multiplica por subtarefa)
      ...
      LOOP
        ...
        IF v_child_rec.tags IS NOT NULL AND array_length(v_child_rec.tags, 1) > 0 THEN
          ...
          LOOP
            ...
            END IF;
            -- Penalidade por etiqueta: aplicada UMA vez por etiqueta encontrada (não multiplicada por subtarefa)
            -- Aqui acumulamos no nível do pai, mas SEM multiplicar por subtarefa: somamos só uma vez por subtarefa que tem essa tag
            -- Para manter o comportamento "-1 na tarefa toda", ignoramos late_penalty por etiqueta filha.
          END LOOP;
        END IF;

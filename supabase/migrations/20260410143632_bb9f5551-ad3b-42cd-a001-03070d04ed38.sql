...
BEGIN
  ...
  ELSE
    -- ═══ NO SUBTASKS: create one task per assignee (assignee_id + watchers) ═══
    -- Build deduplicated list of all assignees
    ...
    IF v_watchers IS NOT NULL AND array_length(v_watchers, 1) > 0 THEN
      ...
      END LOOP;
    END IF;
    -- Fallback: if no assignees at all, use _user_id
    IF array_length(v_all_assignees, 1) IS NULL OR array_length(v_all_assignees, 1) = 0 THEN
      v_all_assignees := ARRAY[_user_id];
    END IF;

    -- Delete old single-key format task (from before this fix)
    DELETE FROM public.tasks
    WHERE description = 'pm:' || _pm_task_id::text || ':' || _completed_stage
      AND deleted_at IS NULL;

    -- Create one task record per assignee
  ...
  -- ═══ RECALCULATE TAG POINTS first, then recompute scores ═══

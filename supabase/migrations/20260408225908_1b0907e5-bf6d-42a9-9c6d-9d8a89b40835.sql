...
BEGIN
  ...
  LOOP
    ...
    -- Check if this is a tag-only stage (design/edicao_videos)
    ...
    -- For tag-only stages, force defaults even if config is missing
    IF v_is_tag_only_stage THEN
      v_stage_base := COALESCE(v_stage_base, 0);
      v_stage_uses_qty := true;
      v_stage_extra_mult := COALESCE(v_stage_extra_mult, 1.5);
    ELSE
    ...
    IF v_stage_uses_qty THEN
      -- Try child PM tasks first (filtered by assignee if available)
      ...
      -- Fallback: try all child PM tasks (no assignee filter)
      ...
      -- Fallback: use parent task's own tags
      IF v_tags IS NULL OR array_length(v_tags, 1) IS NULL THEN
        v_tags := v_parent_tags;
      END IF;

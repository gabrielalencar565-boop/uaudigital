...
BEGIN
  ...
  LOOP
    ...
    IF v_is_tag_only_stage THEN
      ...
      IF v_stage_extra_mult IS NULL OR v_stage_extra_mult <= 0 THEN
    ...
    IF v_stage_uses_qty THEN
      -- Determine scoring assignee from description key
      v_scoring_assignee := NULL;
      IF array_length(v_desc_parts, 1) >= 4 THEN
        v_scoring_assignee := v_desc_parts[4]::uuid;
      END IF;

      -- Count children for this assignee
      v_child_count := 0;

      -- Calculate per-child: iterate each child and sum its individual tag points
      FOR v_child_rec IN
      ...
      LOOP
        ...
        -- Each child contributes: stage_base + its own tag points
        v_all_children_sum := v_all_children_sum + v_stage_base + v_child_tag_sum;
      END LOOP;

      -- If no children found, fallback to parent tags × snapshot quantity
      ...
      -- Apply extra demand multiplier
      v_total_points := v_all_children_sum
        * CASE WHEN v_is_extra THEN v_stage_extra_mult ELSE 1 END;

    ELSE
      -- Non-quantity stages: simple base points

...
BEGIN
  ...
  -- This ensures all watchers also receive scoring via recompute_metas_prazos
  IF v_watchers IS NOT NULL AND array_length(v_watchers, 1) > 0 THEN
    FOREACH v_watcher IN ARRAY v_watchers
    LOOP
      -- Skip if watcher is already the main assignee
  ...
  -- Also add the pm_task assignee if different from _user_id (the completer)
  ...
  -- Recompute performance scores for the main user
  PERFORM public.recompute_all_scores(_user_id, v_year, v_month);

  -- Recompute for all watchers too
  IF v_watchers IS NOT NULL AND array_length(v_watchers, 1) > 0 THEN
    FOREACH v_watcher IN ARRAY v_watchers
    LOOP
      IF v_watcher <> _user_id THEN
        PERFORM public.recompute_all_scores(v_watcher, v_year, v_month);
      END IF;
    END LOOP;
  END IF;

  -- Recompute for assignee if different
  IF v_assignee_id IS NOT NULL AND v_assignee_id <> _user_id THEN

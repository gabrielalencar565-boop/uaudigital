-- 1) pm_recalc_tag_points: late penalty for tag-based stages = single -1 (not sum of tag penalties)
...
BEGIN
  ...
  LOOP
    ...
    IF v_is_tag_only_stage THEN
      ...
      END IF;
      -- Single -1 penalty on the parent regardless of tags
      v_stage_late_penalty := -1;
    ELSE
...
-- 2) pm_sync_stage_completion: skip revisao and alteracoes entirely (no snapshot, no score)
...
DECLARE
  ...
  v_primary_user uuid;
BEGIN
  -- Revisão e Alterações não pontuam
  IF _completed_stage IN ('revisao', 'alteracoes') THEN
    RETURN;
  END IF;

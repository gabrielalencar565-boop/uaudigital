...
BEGIN
  ...
  v_is_subtask := (v_parent_task_id IS NOT NULL);

  -- For subtasks: inherit parent's tags if subtask has none, and use parent's due_date if missing
  IF v_is_subtask THEN
    IF v_tags IS NULL OR array_length(v_tags, 1) IS NULL THEN
      SELECT tags INTO v_tags FROM public.pm_tasks WHERE id = v_parent_task_id;
    END IF;
    IF v_due_date IS NULL THEN
      SELECT due_date INTO v_due_date FROM public.pm_tasks WHERE id = v_parent_task_id;
    END IF;
  ELSE
    -- For parent tasks: if parent has no tags, aggregate unique tags from subtasks
    IF v_tags IS NULL OR array_length(v_tags, 1) IS NULL THEN
  ...
  -- For subtasks, quantity is always 1; for parent tasks, count children
  IF v_is_subtask THEN
    v_quantity := 1;
  ELSE
  ...
  -- Magic sync (only for parent tasks, not subtasks)
  IF NOT v_is_subtask AND NOT v_is_extra THEN
    BEGIN
...
-- Also update pm_recalc_tag_points to check subtask tags when parent has none
...
BEGIN
  ...
  -- If parent has no tags, aggregate from subtasks
  IF v_tags IS NULL OR array_length(v_tags, 1) IS NULL THEN
    SELECT array_agg(DISTINCT t_tag)
    INTO v_tags
    FROM public.pm_tasks sub, unnest(sub.tags) AS t_tag

...
BEGIN
  ...
  -- Fallback if no subtask assignees
  ...
  -- PERFORMANCE SYNC — score each user by THEIR subtask count AND tag-based points
  FOREACH v_uid IN ARRAY v_all_users LOOP
    -- Calculate tag-based points for this user's subtasks
    v_tag_points := 0;
    v_user_quantity := 0;
    ...
    LOOP
      ...
      -- Check each tag of the subtask for scoring config
      IF v_child_tag.tags IS NOT NULL AND array_length(v_child_tag.tags, 1) > 0 THEN
        FOREACH v_tag_name IN ARRAY v_child_tag.tags LOOP
          -- Extract tag name (before the last colon, e.g., "Post:blue" -> "Post")
          v_tag_key := 'tag_' || lower(regexp_replace(split_part(v_tag_name, ':', 1), ' ', '_', 'g'));
          -- Look up scoring config for this tag
    ...
    IF v_existing_task_id IS NULL THEN
      INSERT INTO public.tasks (
        ...
        now(),
        v_is_extra,
        CASE WHEN v_tag_points > 0 THEN v_tag_points ELSE NULL END
      );
    ELSE
      UPDATE public.tasks
      SET status = 'concluido'::public.task_status,
          ...
          updated_at = now(),
          point_value = CASE WHEN v_tag_points > 0 THEN v_tag_points ELSE NULL END

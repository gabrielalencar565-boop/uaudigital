...
-- Recompute todos os scores do mês atual para refletir imediatamente
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT DISTINCT user_id, EXTRACT(YEAR FROM due_date)::int AS y, EXTRACT(MONTH FROM due_date)::int AS m
    FROM (
      SELECT t.assigned_user_id AS user_id, t.due_date FROM public.tasks t
      WHERE t.description LIKE 'pm:%' AND split_part(t.description,':',3) LIKE 'custom_%'
        AND t.status='concluido' AND t.deleted_at IS NULL AND t.assigned_user_id IS NOT NULL
      UNION
      SELECT ta.user_id, t.due_date FROM public.tasks t
      JOIN public.task_assignees ta ON ta.task_id = t.id
      WHERE t.description LIKE 'pm:%' AND split_part(t.description,':',3) LIKE 'custom_%'
        AND t.status='concluido' AND t.deleted_at IS NULL
    ) s
    WHERE user_id IS NOT NULL AND due_date IS NOT NULL
  LOOP
    PERFORM public.recompute_all_scores(r.user_id, r.y, r.m);
  END LOOP;
END $$;
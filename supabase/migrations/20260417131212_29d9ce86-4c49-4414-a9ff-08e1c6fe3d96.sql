...
BEGIN
  ...
  LOOP
    ...
    -- Penalidade da ETAPA aplicada UMA VEZ na tarefa pai
    v_parent_late_sum := v_stage_late_penalty;
    -- Coleta de etiquetas únicas (para somar penalidade uma vez por etiqueta no total)
    ...
    IF v_stage_uses_qty THEN
      ...
      LOOP
        ...
        IF v_child_rec.tags IS NOT NULL AND array_length(v_child_rec.tags, 1) > 0 THEN
          ...
          LOOP
            ...
            -- Coleta de etiquetas únicas para penalidade
      ...
      -- Soma a penalidade de cada etiqueta UMA vez no total (não por subtarefa)

-- 1) Fix pm_sync_stage_completion to use the task's assignee, not the clicker
CREATE OR REPLACE FUNCTION public.pm_sync_stage_completion(_pm_task_id uuid, _completed_stage text, _user_id uuid DEFAULT NULL::uuid, _scoring_user_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- The score should always go to the task assignee, not whoever clicked "Concluído".
  v_primary_user := COALESCE(v_assignee_id, _user_id);
  ...
  ELSE
    IF v_force_parent_assignee THEN
      v_all_users := ARRAY[v_primary_user];
    ELSE
      IF _scoring_user_ids IS NOT NULL AND array_length(_scoring_user_ids, 1) > 0 THEN
        ...
        WHERE u IS NOT NULL;
      ELSE
        -- Always credit the task assignee (not whoever pressed the button)
        v_all_users := ARRAY[v_primary_user];
        IF v_watchers IS NOT NULL AND array_length(v_watchers, 1) > 0 THEN
          FOREACH v_watcher IN ARRAY v_watchers LOOP
...
-- 2) Fix the existing wrong snapshot for the Arco Íris extra demand
UPDATE public.tasks
SET assigned_user_id = 'ee7f3bfd-0de8-4612-bb1d-acbcef2c600f',
    description = 'pm:dcea51e4-531b-42ac-a589-6cf4532e6487:edicao_videos:ee7f3bfd-0de8-4612-bb1d-acbcef2c600f'
WHERE id = 'f1c66244-2773-400b-b25d-ddf32d88b148'
  AND description = 'pm:dcea51e4-531b-42ac-a589-6cf4532e6487:edicao_videos:36d3446d-2d2a-46dc-a4b6-e8c8b26db553';

-- 3) Recompute scores for both users in June 2026 so Kauê gains and Isac loses
SELECT public.recompute_metas_prazos('ee7f3bfd-0de8-4612-bb1d-acbcef2c600f'::uuid, 2026, 6);
SELECT public.recompute_metas_prazos('36d3446d-2d2a-46dc-a4b6-e8c8b26db553'::uuid, 2026, 6);
...
export function Magic2Checklist({ cycles, scheduledByClient, isLoading }: Props) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className={cn(isMobile ? "text-base" : "text-lg")}>Checklist de agendamento</CardTitle>
        <CardDescription>
          ...
          <div className="space-y-3">
            {sortedCycles.map((c) => {
              ...
              return (
                <div
                  key={c.id}
                  className="rounded-lg border border-border/60 bg-card/10 p-3 sm:p-4"
                >

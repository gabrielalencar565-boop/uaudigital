    ...
    <div className="flex flex-col h-[480px]">
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-2 rounded-lg border border-border/40 p-3 bg-muted/20">
        ...
        {messages.data?.map((m: any) => {
          ...
          const del = async () => {
            const { error } = await supabase.from("whatsapp_messages").delete().eq("id", m.id);
          ...
          const deleteBtn = (
            <AlertDialog>
              <AlertDialogTrigger asChild>
                <button type="button" className="opacity-0 group-hover:opacity-100 transition text-muted-foreground hover:text-destructive p-1" title="Apagar mensagem">
            ...
            <div key={m.id} className={cn("group flex items-center gap-1.5", own ? "justify-end" : "justify-start")}>
              ...
              <div
                className={cn(
                  ...
                  own ? "bg-primary text-primary-foreground" : "bg-card border border-border/50",
                )}
              >

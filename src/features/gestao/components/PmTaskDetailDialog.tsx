...
function TaskContentView({
  ...
  const handleSetCover = (url: string) => {
    updateTask.mutate({ id: task.id, cover_url: url } as any);
    toast.success("Capa definida!");
  };

  const handleRemoveCover = () => {
    updateTask.mutate({ id: task.id, cover_url: null } as any);
      ...
      {task.cover_url && (
        <div className="relative w-full h-40 overflow-hidden bg-muted">
          ...
          <Button
            size="sm"
            variant="secondary"
            className="absolute top-2 right-2 h-6 text-[10px] opacity-0 hover:opacity-100 focus:opacity-100 transition-opacity"
      ...
      <div className="px-6 py-5 space-y-6">
        ...
        <div className="border-t border-border/20 pt-4">
          <PmAttachmentsSection
            ...
            currentCoverUrl={task.cover_url}
          />
        </div>
      </div>
    </div>
